...
async function main() {
  ...
  let server = getArg('--server');
  let nick = getArg('--nick');
  if (!server) server = await ask(rl, 'Adres serwera (np. https://twoja-nazwa.onrender.com): ');
  if (!nick) nick = await ask(rl, 'Twoj nick: ');
